    ...
    function menuToggle() {
        ...
        if ($('body').hasClass('modal-open')) {
            if (hasScrollbar()) {
                ...
                $('.navbar-dark.fixed-top').css('margin-right', '');
                $('.align-items-end.sites-tabs-container-top').css('padding-right', '');
            }
            $('#menu-bg').fadeOut(800, function () {
                $(this).remove();
            });
        } else {
            var _w = getScrollbarWidth();
            if (hasScrollbar()) {
                $('body').css('padding-right', _w);
                $('.navbar-dark.fixed-top').css('margin-right', _w);
                $('.align-items-end.sites-tabs-container-top').css('padding-right', _w);
            }
    ...
    $(document).on('focus', '.smart-tips.search-key', function () {
        ...
        if (_this.val() == '') {
            ...
            if (list.find('i').length > 0) {
                ...
                loadUrlData = setTimeout(function () {
                    showUrlBox(_this, parent);
                }, 500);
            }
            ...
            .done(function (response) {
                ...
                if (tipsList) {
                    for (var i = 0; i < tipsList; i++) {
                        list.children('ul')
                            .removeClass('li-null')
                            .append("<li><a href='" + response[i]['url'] + "' target='_blank' rel='external nofollow noopener'><i class='iconfont icon-globe mr-2'></i>" + response[i]['url_name'] + '<a></li>');
                    }
                    list.slideDown(200);
                } else {
                    list.slideUp(200);
